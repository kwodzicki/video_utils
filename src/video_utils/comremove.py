...
# ffmpeg -copyts -i "concat:in1.ts|in2.ts" -muxpreload 0
# -muxdelay 0 -c copy joint.ts
...
class ComRemove:
    ...
    def comskip(self, in_file: str, name: str = '') -> str | None:
        """
        ...
        # Wait for 8 hours for comskip to finish; this should be more
        # than enough time
        ...
        if proc.returncode == 1:
            ...
            self.__log.warning(
                'No EDL file was created; trying to convert TXT file'
            )
            edl_file = self.convert_txt(txt_file, edl_file)
        file_remove(txt_file, logo_file)
        return edl_file

    def comchapter(self, in_file: str, edl_file: str) -> str:
        """
        ...
        segment = 1
        commercial = 1
        # Initial start time of the show segment;
        # i.e., the beginning of the recording
        seg_start = 0.0
        ...
        with open(edl_file, mode='r') as fid:
            ...
            while info:
                ...
                # If the start of the commercial is NOT near the very
    ...
    def comcut(self, in_file: str, edl_file: str) -> list[str] | None:
        """
        ...
        fnum = 0
        # Initial start time of the show segment; i.e., the beginning
        # of the recording
        ...
        with open(edl_file, 'r') as fid:
            ...
            while info:
                ...
                if com_start.total_seconds() > 1.0:
                    ...
                    procs.append(
                        POPENPOOL.popen_async(cmd, threads=1)
                    )
    ...
    def comjoin(self, tmpfiles: list[str]) -> str | None:
        """
    ...
    def check_size(self, in_file: str, cut_file: str) -> None:
        """
        ...
        then just replace the input file with the cut file (one with no
        commercials). If the file size is NOT sane, then the cut file is
        ...
        self.__log.debug(
            "Running file size check to make sure too much wasn't removed"
        )
        ...
        if 1.1 > float(cut_file_size) / float(in_file_size) > 0.5:
            msg = (
                'Output file size looked sane, replacing the original: '
                '%s -> %s'
            )
            replace = True
        elif 1.01 > float(cut_file_size) / float(in_file_size) > 0.99:
            msg = (
                'Output file size was too similar; keeping original: '
                '%s -> %s'
            )
        else:
            msg = (
                'Output file size looked odd (too big/too small); '
                'keeping original: %s -> %s'
            )
        self.__log.info(
    ...
    def convert_txt(self, txt_file: str, edl_file: str) -> str | None:
        """
    ...
    def _get_ini(self, name: str = '') -> str | None:
        """
    ...
    ########################################################
    def __size_fmt(self, num: int, suffix: str = 'B') -> str:
        """
        Size of a file in a human readable format
