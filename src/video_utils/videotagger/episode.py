...
class BaseEpisode(BaseItem):
    ...
    def get_basename(self, **kwargs) -> str:
        """
    ...
    def get_dirname(self, root: str = '') -> str:
        """
...
class TMDbEpisode(BaseEpisode):
    ...
    def __init__(self, *args, **kwargs):
        """
        Arguments:
            seriesID (int, TMDbSeries): The series ID from themoviedb.com,
                OR a Series object.
        ...
        if not self._data:
            if len(args) < 3:
                raise Exception(
                    "Must input series ID or object and season and "
                    "episode number"
                )
...
class TVDbEpisode(BaseEpisode):
    ...
    def __init__(self, *args, **kwargs):
        """
        Arguments:
            seriesID (int, TVDbSeries): The series ID from themoviedb.com,
                OR a Series object.
            seasonNum (int): Season number of episode
            episodeNum (int): Episode number of episode

        Keyword arguments:
            dvdOrder (bool): Set to use dvdOrder from TVDb;
                default is airedOrder
            **kwargs: Various, none used
        ...
        if len(args) < 3:
            raise Exception(
                "Must input series ID or object and season and episode number",
            )
        ...
        if dvdOrder:
            ...
            if json is None:
                self.__log.warning(
                    'TVDb search based on DVD order failed, '
                    'falling back to aired order'
                )
