...
class SearchWidget(QWidget):
    ...
    def register_func(self, func):
        ...
        Arguments:
            func : Function reference to register to the search button
    ...
    def search(self, *args, **kwargs):
        ...
        Returns:
            An instance of either TVDbMovie, TVDbEpisode, TMDbMovie,
                TMDbEpisode based on search criteria. If bad criteria,
                then returns None.
...
class MetadataWidget(QWidget):
    ...
    def _write_data(self):
        ...
        if cur_file == '':
            dialog = QFileDialog()
            cur_file = dialog.getOpenFileName(
                directory=HOME,
                filter=EXT_FILTER,
            )[0]
            if cur_file == '':
