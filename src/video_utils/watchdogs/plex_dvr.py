...
class PlexDVRWatchdog(FileSystemEventHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for arg in args:
            # Add each input argument to observer as directory to watch
    ...
    def on_created(self, event):
        ...
        # Acquire Lock so other events cannot change to_convert list at
        # same time
        with self.__lock:
    ...
    def check_recording(self, fpath):
        ...
        Arguments:
            fpath (str): Path to newly created file from event in
                on_created() method
        ...
        Returns:
            bool: True if file is a recording (i.e., it's just been moved),
                False otherwise

        """

        # Acquire Lock so other events cannot change to_convert
        # list at same time
        with self.__lock:
            t_start = time.time()
            ...
            while i < len(self.recordings):
                ...
                if self.recordings[i][1] == fname:
                    ...
                    # Append to converting list; this will trigger
                    # update of queue file
                ...
                if time_delta > self.record_timeout:
                    self.log.info(
                        'File is more than %0.0f s old, '
                        'assuming record failed: %s',
                        time_delta,
...
def cli():
    ...
    desc = (
        'A CLI for running a watchdog to monitor a Plex library '
        '(or libraries) for new files to convert to h264 encoded videos'
    )
